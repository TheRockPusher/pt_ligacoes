...
3. Verify worker startup and process liveness. `APP_PROCESS=import-worker` selects the worker probe rather than the web HTTP probe. The worker refuses to claim jobs with unavailable/pending schema; it does not run migrations. Railway's bounded process-restart policy is not an import retry policy. If a rollout race exhausts startup restarts, first verify successful web migrations, then authorise a worker restart. Existing queued jobs remain durable.
