...
5. **GitHub → deploy**: código não confiável de PR não recebe segredos de produção. O job de release com escrita não faz checkout/execução de código do PR. GitHub CI e Railway têm configurações externas que têm de ser verificadas.
