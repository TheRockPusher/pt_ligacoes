...
The live `main` protection requires both `ci` and `CodeQL`, with up-to-date PRs and no administrator bypass. Release Please's default token needs a maintainer close/reopen after each bot update to start PR checks. Verify that the generated release changes the root package in `uv.lock` as well as `pyproject.toml`; the pinned TOML updater's tagged-scalar selector is explained in `docs/operations.md`.
